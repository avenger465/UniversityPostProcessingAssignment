...
float4 main(PostProcessingInput input) : SV_Target
{
	...
	float centreLengthSq = dot(centreVector, centreVector) / 1.5;
	...
	float SinX = sin(input.areaUV.x * radians(250.0f) + gHeatHazeTimer * 3.0f);
	float SinY = sin(input.areaUV.y * radians(250.0f) + gHeatHazeTimer * 3.5f);
    ...
    float3 UnderWaterColourTint = { 0.31f, 0.26f, 0.71f };
