...
const std::array<CVector3, 4> HeartWindowPoints = { { {-4.5,5,0}, {-4.5,-5,0}, {4.5,5,0}, {4.5,-5,0} } };
const std::array<CVector3, 4> SpadeWindowPoints = { { {-4.5,5,0}, {-4.5,-5,0}, {4.5,5,0}, {4.5,-5,0} } };

const std::array<CVector3, 4> DiamondWindowPoints = { { {-3,3,0}, {-4,-4,0}, {4,4,0}, {3,-3,0} } };

const std::array<CVector3, 4> CloverWindowPoints = { { {-4.5,5,0}, {-4.5,-5,0}, {4.5,5,0}, {4.5,-5,0} } };
const std::array<CVector3, 4> SquarePoints = { { {-5,5,0}, {-5,-5,0}, {5,5,0}, {5,-5,0} } };
//****************************
...
bool InitGeometry()
{
	...
	try
	{
		gStarsMesh  = new Mesh("Data/Stars.x");
		gGroundMesh = new Mesh("Data/Ground.x");
		gCubeMesh   = new Mesh("Data/Cube.x");
		gCrateMesh  = new Mesh("Data/Wall1.x");
		gCrate1Mesh  = new Mesh("Data/Wall2.x");
		gLightMesh  = new Mesh("Data/Light.x");
	}
	...
	if (!LoadTexture("Media/Stars.jpg",                &gStarsDiffuseSpecularMap,  &gStarsDiffuseSpecularMapSRV) ||
		!LoadTexture("Media/brick_35.jpg",                &gBrickDiffuseSpecularMap,  &gBrickDiffuseSpecularMapSRV) ||
		!LoadTexture("Data/GrassDiffuseSpecular.dds", &gGroundDiffuseSpecularMap, &gGroundDiffuseSpecularMapSRV) ||
		!LoadTexture("Data/StoneDiffuseSpecular.dds", &gCubeDiffuseSpecularMap,   &gCubeDiffuseSpecularMapSRV) ||
		!LoadTexture("Data/CargoA.dds",               &gCrateDiffuseSpecularMap,  &gCrateDiffuseSpecularMapSRV) ||
...
bool InitScene()
{
	...
	gCrate1  = new Model(gCrate1Mesh);

	// Initial positions
	gCube->SetPosition({ 50, 5, -10 });
	gCube->SetRotation({ 0.0f, ToRadians(-110.0f), 0.0f });
	gCube->SetScale(1.5f);
	gCrate->SetPosition({ 41.85, 0, 36.9 });
	gCrate->SetRotation({ 0.0f, ToRadians(90.0f), 0.0f });
	gCrate->SetScale(40.0f);


	gCrate1->SetPosition({ 10, 0, 68 });
	
	gCrate1->SetScale(40.0f);


	gStars->SetScale(8000.0f);

	polyMatrix = MatrixTranslation({ gCrate->Position().x, 10, gCrate->Position().z });
	polyMatrix = MatrixRotationY(ToRadians(90)) * polyMatrix;

	HeartMatrix = MatrixTranslation({ gCrate1->Position().x + 18, 10, gCrate1->Position().z});

	SpadeMatrix = MatrixTranslation({ gCrate1->Position().x - 18, 10, gCrate1->Position().z});

	DiamondMatrix = MatrixTranslation({ gCrate1->Position().x - 5.5f, 10, gCrate1->Position().z});
	DiamondMatrix = MatrixRotationZ(ToRadians(45)) * DiamondMatrix;

	CloverMatrix = MatrixTranslation({ gCrate1->Position().x + 6, 10, gCrate1->Position().z});
...
void ReleaseResources()
{
	...
	delete gCrate1;   gCrate1 = nullptr;
...
void RenderScene()
{
	...
	if (gCurrentPostProcess != PostProcess::None)
	{
		...
		else if (gCurrentPostProcessMode == PostProcessMode::Area)
		{
			...
			AreaPostProcess(gCurrentPostProcess, CVector3{gCrate->Position().x, 10,gCrate->Position().z }, { 5, 5 });
		}
		...
		else if (gCurrentPostProcessMode == PostProcessMode::Polygon)
		{
			...
			//static CMatrix4x4 polyMatrix = MatrixTranslation({ gCrate->Position().x, 10, gCrate->Position().z});
			//polyMatrix = MatrixRotationY(ToRadians(1)) * polyMatrix;
			
			// Pass an array of 4 points and a matrix. Only supports 4 points.
			PolygonPostProcess(gCurrentPostProcess, SquarePoints, polyMatrix);
		}
