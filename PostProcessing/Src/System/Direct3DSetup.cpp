...
bool InitDirect3D(int ViewportWidth, int ViewportHeight, HWND HWnd, std::string LastError)
{
    ...
    swapDesc.BufferDesc.Height = ViewportHeight;            // --"--
    ...
    if (FAILED(hr))
    {
        LastError = "Error creating Direct3D device";
        return false;
    }
    ...
    if (FAILED(hr))
    {
        LastError = "Error creating swap chain";
        return false;
    }
    ...
    if (FAILED(hr))
    {
        LastError = "Error creating render target view";
        return false;
    }
    ...
    dbDesc.Width  = ViewportWidth; // Same size as viewport / back-buffer
    dbDesc.Height = ViewportHeight;
    dbDesc.MipLevels = 1;
    dbDesc.ArraySize = 1;
